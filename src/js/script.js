 const html              = document.documentElement;
    ...
    const students          = window.passaporteData?.alunos ?? [];

    lucide.createIcons();

    const modulosPadrao = students[0]?.modulos ?? [
    ...
    let activeStudent = students[0] ?? {modulos: modulosPadrao, progresso_insignias: {total: 0}};
    let modulos;

    try{
        const salvo = localStorage.getItem(`ltd-modulos-${activeStudent.email}`);
    ...
    function salvarModulos(){
        localStorage.setItem(`ltd-modulos-${activeStudent.email}`, JSON.stringify(modulos))
    }
    ...
    function updateStudentProfile(){
        const name = activeStudent.nome || "Aluno";
        ...
        if(studentName) studentName.textContent = `Olá, ${name}`;
    }

    function activateStudent(student){
        activeStudent = student;
        const salvo = localStorage.getItem(`ltd-modulos-${activeStudent.email}`);
        modulos = salvo ? JSON.parse(salvo) : activeStudent.modulos;
        currentModuleId = modulos.find(m => m.status === "conquistada")?.id || 1;
